// อะไหล่ ระบบจุดระเบิด/ห้องเครื่อง ไม่ด่วน


/// <reference types="cypress" />

context("workshop-OnlinePartsIgnition", () => {

    it("Add order-parts", () => {
        cy.login("empGrip01", "password")
        orderOnlineIgnition()
        orderOnlineIgnition1()
        checkorderOnlineIgnition()
        logout()
    })
...
const orderOnlineIgnition1 = () => {
    cy.get('#selSearchPart')
        .wait(2000).click().type("ยางแท่นเครื่อง").type("{enter}")

    cy.get('#vs12__combobox > .vs__selected-options > #selSearchPartBrands')
        .wait(2000).click().type("RBI").type("{enter}")


    cy.get('#btnAddCartById-6059')
        .click()
...
const checkorderOnlineIgnition = () => {
