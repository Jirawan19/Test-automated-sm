...
const Addorderworkshop2 = () => {
    ...
    cy.get('.col-xl-4 > .el-select > .el-input > .el-input__inner')
        .click().type("บริษัท ออโต้แพร์ จำกัด").type("{downarrow}{enter}")
    ...
    // // อะไหล่
    cy.get(':nth-child(1) > :nth-child(2) > label').should("contain.text", "ชื่อสินค้า")
    cy.get('#productname').click().type("ปีกนก", "{downarrow}{enter}")

    // // ค้นหา
    cy.get(':nth-child(3) > :nth-child(1) > .btn-search').should("contain.text", "ค้นหา")
    cy.get(':nth-child(3) > :nth-child(1) > .btn-search').click()

    // // เลือกสินค้า
    cy.get('#pane-1 > .d-xl-block > .table > thead > tr > :nth-child(1)')
        .should("contain.text", "รหัสสินค้า")
    cy.get('.d-xl-block > .table > tbody > :nth-child(1) > :nth-child(1)')
        .should("contain.text", "ATP-PS0129R")
    cy.get('#pane-1 > .d-xl-block > .table > thead > tr > [style="width: 200px;"]')
        .click()
    cy.get(':nth-child(1) > :nth-child(6) > .btn-details')
        .click({ force: true })
    cy.get('.close').click()
    ...
    cy.get('.col-12.d-none > .table > tbody > tr > :nth-child(2) > .primary-blue')
        .should("contain.text", "ATP-PS0129R")

    cy.get('.col-12.d-none > .table > thead > tr > :nth-child(4)')
        .should("contain.text", "จำนวน")
    cy.get(':nth-child(4) > .form-check > .form-control')
        .clear().type("3")

    cy.get('.col-12.d-none > .table > thead > tr > :nth-child(5)')
        .should("contain.text", "ราคาต่อหน่วย")
    cy.get('.col-12.d-none > .table > tbody > tr > :nth-child(5) > :nth-child(5)')
        .should("contain.text", "1,170.00")
    ...
    cy.get('.col-12.d-none > .table > tfoot > :nth-child(3) > .text-right')
        .should("contain.text", "3,755.70 บาท")
...
const checkAddopenorder2 = () => {
    ...
    cy.get('tbody > :nth-child(4) > :nth-child(3)')
        .should("contain.text", "245.70 บาท")
